...
function Status() {
  ...
  return (
    <div className="border-double border-8 p-4 bg-floral-white text-taupe-gray space-y-2">
      <div className="flex items-center space-x-3">
        <img
          className="h-20 w-20 rounded-full justify-end border-4 border-taupe-gray"
          src={post?.avatar}
        />
        <div className="flex flex-col">
          <a
            className="text-sm font-semibold"
            href="https://bsky.app/profile/neatbee.bsky.social"
          >
            @{post?.handle}
          </a>
          <p className="text-xs text-gray-500">Posted on {post?.getDate()}</p>
        </div>
      </div>
      <div className="italic">"{post?.text}"</div>
    </div>
  );
}
